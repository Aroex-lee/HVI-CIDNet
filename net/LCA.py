...
# Mamba
class HV_LCA_with_Mamba(nn.Module):
    def __init__(self, dim,num_heads, bias=False):
        super(HV_LCA_with_Mamba, self).__init__()
        self.gdfn = IEL(dim) # IEL and CDL have same structure
        self.norm = LayerNorm(dim)
        # self.ffn = CAB(dim, num_heads, bias)
        # self.ffn = CrossMambaBlock_Mutli_Heads(dim, num_heads=4, bias=bias)
        self.ffn = CrossMambaBlock(dim, bias=bias)
...
class I_LCA_with_Mamba(nn.Module):
    def __init__(self, dim,num_heads, bias=False):
        super(I_LCA_with_Mamba, self).__init__()
        self.norm = LayerNorm(dim)
        self.gdfn = IEL(dim)
        # self.ffn = CAB(dim, num_heads, bias)
        # self.ffn = CrossMambaBlock_Mutli_Heads(dim, num_heads=4, bias=bias)
...
class CrossMambaBlock_Mutli_Heads_qkv(nn.Module):
...
class CrossMambaBlock_qkv(nn.Module):
...
#         # 新增：用于提取调制向量
#         self.global_pool = nn.AdaptiveAvgPool2d(1)  # 从 y 中生成摘要
...
#         # 从 y 中提取全局摘要 → gate
#         y_summary = self.global_pool(y)  # (b, c, 1, 1)
#         y_gate = self.gate_proj(y_summary)  # (b, c)

#         # reshape gate 以便广播乘法
#         y_gate = y_gate.unsqueeze(-1).unsqueeze(-1)  # (b, c, 1, 1)

#         # 用 y 的 gate 信息调制 q
#         q_modulated = q * y_gate  # (b, c, h, w)

#         # 转换为序列输入
#         seq = rearrange(q_modulated, 'b c h w -> b (h w) c')

#         # Mamba 处理
#         seq_out = self.mamba(seq)  # (b, hw, c)

#         # 转换回空间结构
#         out = rearrange(seq_out, 'b (h w) c -> b c h w', h=h, w=w)
